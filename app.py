...
class TestAdmin(ModelView):
    ...
    def on_model_change(self, form, model, is_created):
        ...
        # Si modification de compte
        else:
            # Ne modifier le mot de passe QUE si le champ est rempli
            if form.password.data:
                model.password = generate_password_hash(form.password.data)
            # Sinon, garder l'ancien mot de passe
            else:
                original_password = self.session.query(Test.password).filter_by(id=model.id).scalar()
