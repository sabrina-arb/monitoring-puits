...
function displaySearchResults(results) {
    ...
    // Ajouter les événements aux boutons "Ouvrir"
...
telegougageForm.addEventListener('submit', (e) => {
    ...
    .then(data => {
        // Recharger les données depuis le serveur après un nouvel enregistrement
        loadInitialData();
...
deleteButton.addEventListener('click', () => {
    if (currentDetailId && confirm("Voulez-vous vraiment supprimer ce télégougage ?")) {
        ...
        .then(data => {
            // Recharger les données depuis le serveur après suppression
            loadInitialData();
            closeModal(detailModal);
        })
        .catch(error => {
...
// Fonction pour exporter toutes les données en Excel
document.getElementById('exportAllButton').addEventListener('click', function() {
    ...
    // Préparer les données pour l'export (en utilisant les mêmes noms que ceux du serveur)
    ...
    // Exporter le fichier avec date dans le nom
